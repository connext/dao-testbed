...
// 0x58b9cb810a68a7f3e1e4f8cb45d1b9b3c79705e8 - Arb NEXT token (token0 on arb vault)

// --------

contract CommunityProposal2 is ForgeHelper {
    enum Operation {
        Call,
        DelegateCall
    }
    ...
    // Transactions path
    string public TRANSACTIONS_PATH = "/community-transaction-2.json";
    ...
    uint256 public LIQUIDITY_AMOUNT_ARB = 442522 ether; // used in transactions
    ...
    function test_executableShouldPass() public {
        ...
        uint256 initialbalance = IERC20(AddressLookup.getNEXTAddress(42161))
            .balanceOf(to);

        // Mint on NEXT to caller
        IXERC20(asset).mint(to, LIQUIDITY_AMOUNT_ARB);
        // No calldata on the xcall
        vm.stopPrank();

        // Ensure the aritrum balance increased
        uint256 balance = IERC20(AddressLookup.getNEXTAddress(42161)).balanceOf(
            to
        );
        assertEq(balance, LIQUIDITY_AMOUNT_ARB + initialbalance, "!balance");
